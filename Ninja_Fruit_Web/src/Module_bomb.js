(function () {
  ...
  var bombUpdate = function () {
    ...
    var px = this.position.x + r * Math.cos(this.rotation - SPP.MathUtils.toRadian(135));
    var py = this.position.y + r * Math.sin(this.rotation - SPP.MathUtils.toRadian(135));
  ...
  var bombExplode = function (target) {
    for (var i = 0; i < 150; i++) {
      var smoke = particleSystem.createParticle(SPP.SpriteImage);
      smoke.init(target.position.x, target.position.y, Infinity, assetsManager.star, topContext);
  ...
  throwBomb = function () {
    var p = bombSystem.createParticle(Fruit);
    ...
    p.init(
      gameWidth * 0.5 + (1 - Math.random() * 2) * 300,
      gameHeight + assetsManager.bomb.height,
      Infinity,
      assetsManager.bomb,
