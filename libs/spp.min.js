...
SPP.EventDispatcher = function () {
  ...
  this.dispatchEvent = function (c) {
    ...
    if (void 0 != d) {
      c.target = this;
      b = d.slice();
      for (var d = 0, e = b.length; d < e; d++) b[d].call(this, c);
    }
  };
};
...
SPP.Particle.prototype = {
  ...
  init: function (a, b, c) {
    0 >= c ? this.dispatchEvent(new SPP.Event("dead")) : ((this.life = c || Infinity), this.position.reset(a, b));
  },
  addForce: function (a, b) {
    this.forcesMap[a] = b;
  },
  removeForce: function (a) {
    delete this.forcesMap[a];
  },
  removeAllForces: function () {
    for (var a in this.forcesMap) delete _forces[a];
  },
  render: function () {
    this.update();
    this.onUpdate && this.onUpdate.apply(this);
    this.resultant.reset(0, 0);
    for (var a in this.forcesMap)
      this.forcesMap[a].isActive()
        ? this.resultant.add(this.forcesMap[a].value)
        : (delete this.forcesMap[a], (this.forcesMap[a] = null));
    this.velocity.add(this.resultant);
    this.velocity.x *= 1 - this.damp.x;
    this.velocity.y *= 1 - this.damp.y;
    this.position.add(this.velocity);
    if (null != this.boundary) this[this.boundaryType]();
    this.life -= SPP.frameTime;
    0 < this.life || this.dispatchEvent(new SPP.Event("dead"));
  },
  update: function () {},
  bounce: function () {
  ...
  offscreen: function () {
    this.position.x < this.boundary.left() && (this.position.x = this.boundary.right());
    this.position.x > this.boundary.right() && (this.position.x = this.boundary.left());
    this.position.y < this.boundary.top() && (this.position.y = this.boundary.bottom());
    this.position.y > this.boundary.bottom() && (this.position.y = this.boundary.top());
  },
  destory: function () {},
  reset: function () {
  ...
  toString: function () {
    return (
      ...
      ",\n\tresultant:" +
      this.resultant.toString() +
      ",\n\tboundaryType:" +
      this.boundaryType.toString() +
      ",\n\tboundary:" +
      (null != this.boundary ? this.resultant.toString() : "null") +
      ",\n\tbounceIntensity:" +
      this.bounceIntensity +
      ",\n\tdamp:" +
      this.damp.toString() +
      ",\n\textra:" +
      this.extra.toString() +
      ",\n\tonUpdate:" +
      (null != this.onUpdate ? this.onUpdate.toString() : "null") +
      "\n]\n"
    );
  },
};
...
SPP.ParticleSystem = function () {
  var a = [],
    b = new SPP.ParticlePool(),
    c = null,
    d = !1;
  this.getParticles = function () {
    return a;
  };
  this.createParticle = function (c) {
    c = b.get(c);
    c.reset();
    a.push(c);
    return c;
  ...
  this.render = function () {
    if (d) {
      SPP.frameTime = 0.001 * (Date.now() - c);
      c = Date.now();
      for (var e = a.length, f = 0; f < e; f++) a[f].render();
      for (; 0 < e--; ) 0 >= a[e].life && (b.recycle(a[e]), a.splice(e, 1));
    }
  };
  this.start = function () {
    c = Date.now();
...
SPP.Force = function (a, b, c) {
  SPP.EventDispatcher.call(this);
  this.value = new SPP.Vector2D(a, b);
  this.life = c || Infinity;
};
SPP.Force.prototype = {
  constructor: SPP.Force,
  isActive: function () {
    if (0 >= (this.life -= SPP.frameTime)) return this.destory(), dispatchEvent(new SPP.Event("dead")), !1;
    this.update();
    return !0;
  },
  update: function () {},
};
SPP.Brownian = function (a, b, c) {
  SPP.Force.call(this, 0, 0, c);
  this.maxValue = a;
  this.cycle = b;
  this.pastTime = 0;
...
SPP.Gravity = function (a) {
  SPP.Force.call(this, 0, a, Infinity);
};
SPP.Gravity.prototype = SPP.inherit(SPP.Force.prototype);
SPP.Gravity.prototype.constructor = SPP.Gravity;
SPP.Attraction = function (a, b, c, d) {
  SPP.Force.call(this, 0, 0, d);
  this.maxValue = b;
  this.r = c;
...
SPP.Attraction.prototype.update = function () {
  var a = this.attractionPosition.distanceTo(this.targetParticle.position);
  a < this.r
  ...
  this.value.scale(this.maxValue / a);
};
SPP.Repulsion = function (a, b, c, d) {
  SPP.Force.call(this, 0, 0, d);
  this.maxValue = b;
  this.r = c;
...
SPP.Repulsion.prototype.update = function () {
  var a = this.repulsionPosition.distanceTo(this.targetParticle.position);
  a > this.r
...
SPP.UIDUtil.createUID = function (a) {
...
SPP.SpriteImage.prototype.init = function (a, b, c, d, e) {
